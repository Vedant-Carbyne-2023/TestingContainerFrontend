...
import { currentDate, role, userId } from "../../../CommonUtitlites/Others/commonExportVariable.js";
...
export default function PurchaseOrderForm({vendorId, prId, onDataClick, setToggle}) {
 ...
 useEffect(() => {
  const getVendor = async () => {
    try {
      // Make an API request to get vendor data
      const result = await api.post("/get-vendor-by-id", { userId, role, vendorId });
  ...
  const handleResponse = async () => {
    try {
      // Make an API request to get quotations data
      const result = await api.post("/get-quotations", {
        prId,
        userId,
        role,
      });
      const quotationData = result.data.data;

      // Update the quotation data state
      setQuotationData(quotationData);

      // Update other states based on quotation data
      if (quotationData.length > 0 && quotationData[0].quotations.length > 0) {
        const projectName = quotationData[0].quotations[0].projectName;
  ...
  if (vendorId) {
    // If vendorId is available, fetch vendor data
    getVendor();
  }

  if (prId !== "") {
    // If prId is available, fetch quotation data
    handleResponse();
  }
}, [vendorId, prId]);
...
useEffect(() => {
  ...
  const findAndSetProject = async () => {
    if (project) {
      try {
        const findProject = allProjects.find((projectName) => projectName.name === project);
        if (findProject) {
