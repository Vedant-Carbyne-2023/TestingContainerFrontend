...
export default function UserProjectDashboard({compPermission}) {
  ...
  const showTask = (taskId) => {
    // We will put our api to fetch task with the help of taskID from DB
    // fetch(`/get-task/${taskId}`)
    //   .then((response) => response.json())
    //   .then((data) => setTaskDetails(data));

    // code for getting data from our probable api
    ...
    async function getParticularTask() {
       try{
          setTaskDetails(null);
          let y_result = await api.post(`/get-singleTask`, {taskId:taskId, userId,role});
          y_result = await errorHandler(y_result);
    
          if(y_result.data){
            console.log('got single task', y_result.data.data);
          // setTaskDetails(y_result.data.data);
          // assigned to name
          let name1 = await api.post(`/get-singleUser`, {specificUserId:y_result.data.data[0].taskAssignedTo, userId,role});
          name1 = await errorHandler(name1);
          let name2 = await api.post(`/get-singleUser`, {specificUserId:y_result.data.data[0].userId, userId,role});
    ...
    async function getDetails() {
      ...
      try {
        const [projectRes, tasksRes] = await Promise.all([
          api.post(`/single-project?id=${id}`, { userId, role }),
          api.post(`/get-allTaskOfAProject`, { projectId: id, userId, role }),
        ]);
          ...
          if (task.taskStatus === stat) {
            return (
              <div className="pl-4" key={task.taskId} style={{ marginBottom: '8px' }}>
                {/* <span onClick={() => showTask(task.taskId)} style={{ cursor: 'pointer', textDecoration: 'underline', backgroundColor: task.is_completed ? 'lightgreen' : 'transparent', display: 'block' }}>
                  {task.taskName}
                </span> */}
      ...
      <div className="card">
        <div className="card-header">
          <div className="d-flex justify-content-between align-items-center">
            ...
            <button onClick={onButtonClick} disabled={!permissions.some(permission => permission.edit===true)} className="btn btn-sm btn-outline-primary">
              {buttonText}
            </button>
          </div>
        </div>
