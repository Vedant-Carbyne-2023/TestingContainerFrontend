  ...
  if (columnName === "materialCategory") {
    ...
    newData[rowIndex]["materialDescription"] = data.name;
     ...
     if (Array.isArray(product.uom)) {
      ...
      newData[rowIndex]["uom"] = "nos";
     }
  } else if (columnName === "quantity") {
    newData[rowIndex]["quantity"] = data.target.value;
    newData[rowIndex]["quantity_balance"] = data.target.value;
...
useEffect(() => {
  ...
  if (props.data) {
    const updatedTableData = props.data.map((value, index) => ({
      sNo: index + 1,
      materialCategory: value.materialCategory,
      disabled_materialCategory: false,
      materialSubCategory: value.materialSubCategory,
      disabled_materialSubCategory: false,
      materialDescription: value.materialDescription,
      disabled_materialDescription: false,
      quantity: value.quantity,
      disabled_quantity: false,
      quantity_balance: value.quantity_balance ? value.quantity_balance : value.quantity,
      uom: value.uom,
      disabled_uom: false,
      remark: ""
    }));
              ...
              {tableData.map((row, index) => (
                <tr key={index}>
                  ...
                  <td>
                    <SearchInput
                      id={"Product"}
                      items={products.length>0?products:['No Product Name']}
                  ...
                  <td>
                    ...
                    {validUom &&
                    validUom.map((uom) => (
                      <option value={uom.toLowerCase()}>{uom.charAt(0).toUpperCase() + uom.slice(1)}</option>
                    ))}
  ...
  >
    ...
    {validUom &&
      validUom.map((uom, i) => (
        <option key={i} value={uom.toLowerCase()}>
