...
import {
  api,
  authenticateUser,
} from "../../../CommonUtitlites/AxiosSetup/axiosDefault.js";
...
import PRShowModalEdit from "../ApproveMR_PR/PRShowModalForEdit(Shivansh).jsx";
export default function PR_Represent_Table({ state }) {
  const [status, setStatus] = useState(false);
  const [loading, setLoading] = useState(true);

  let indents = useGetUserPurchaseRequisition(state, status);
  ...
  return (
    <div className="container p-0 mt-3">
      ...
      >
        ...
        <button
          className={`btn ${
            showApproved === "Not Approved" ? "active" : ""
          } ml-2`}
                  ...
                  .map((indent, key) => (
                    <tr key={key}>
                      ...
                      {/* <td>
                        ...
                        <button
                          className="btn btn-link text-left"
                          onClick={() => handleModalOpen2(indent)}
                        >
      ...
      <CustomModal
        ...
        title={"Purchase Requistion Edit"}
      >
        <PRShowModalEdit data={entry} setStatus={(data) => setStatus(data)} />
      </CustomModal>
    </div>
  );
}
