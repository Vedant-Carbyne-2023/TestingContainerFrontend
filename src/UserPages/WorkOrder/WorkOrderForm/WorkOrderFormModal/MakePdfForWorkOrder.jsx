...
const MakePdfForWorkOrder = async(data2) => {
  console.log(data2)
  let data = data2.data
  Font.register({
  ...
  //   setLoading(true)
    ...
    const totalPages = Math.ceil(data.tableData.length / MAX_ROWS_PER_PAGE);
    ...
    function decodeHtml(html, currentIndent = 0) {
      ...
      return elements.map((element, index) => {
        ...
        if (element.tagName === 'P') {
          return (
            <View key={index} style={styles.inputInHtml}>
        ...
        if (element.tagName === 'OL' || element.tagName === 'UL') {
          ...
          return (
            <View
              key={index}
              style={[styles.inputInHtml, { lineHeight: 0.5, marginLeft: 20 * currentIndent }]}
            >
              {decodeHtml(element.innerHTML, currentIndent)}
            </View>
          );
            ...
            <View key={index} style={{ marginLeft: 20 * (currentIndent + childIndent) }}>
              <View style={styles.listItemContainer}>
                ...
                <View style={styles.listItemContentContainer}>
                  <Text style={styles.inputInHtml}>{listItemContent}</Text>
                </View>
              </View>
            </View>
          );
