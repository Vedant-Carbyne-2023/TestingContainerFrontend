...
const [mins, setMins] = useState([])
    useEffect(() => {
    const getMIN = async()=>{
        let result = api.post("/get-min-vendorwise", {userId,role,userName,vendorName})
...
const handleMin = async(minId)=>{
    console.log(minId)
    let result = api.post('/get-single-min', {userId,role,userName,minId})
    result = await errorHandler(result)
...
const handleIndent = async(indentId)=>{
    ...
    let result = api.post('/get-single-indent', {userId,role,userName,indentId})
    result = await errorHandler(result)
