...
export default function OhtApproveDpr  ({ fetchedData, projectName }) {

  const handleOhtApproveRequest = async (item) => {
    let result = await api.post('/approveDailyOhtDpr', { ohtDprId: item, approvedBy: "Vedant As Dpr", status: "Approved" });
  ...
  const handleOhtEditRequest = async () => {
    if(!formData.workDone){
      alert("please enter work Done")
      return
    }

    // let result = await api.post('/approveDailyOhtDpr', { ohtDprId: modalData, approvedBy: "Vedant As Dpr", status: "Approved" });
    // result = await errorHandler(result);

     let result = await api.post(`/editOhtDailyDpr/${modalData}`, 
    { 
      gpName:fetchedData.data[0].gpName,
      typeOfUser:"Dpm",
      projectName:projectName,
      staffName:"Vedant Dpm",
      ohtDate:new Date(fetchedData.data[0].ohtDate),
    ...
    <div className="col-md-12 mt-4">
              ...
              <tr key={item._id}>
                ...
                <td>
                  {item.status === 'Pending For Approval' && (
                    <div>
                    <button className="btn btn-primary" onClick={() => handleOhtApproveRequest(item._id)}>
                      Approve
                    </button>
      ...
      {openModal && (
        <Modal show={openModal} onHide={()=>setOpenModal(false)}>
          ...
          <Modal.Footer>
            <Button variant="primary" onClick={()=>handleOhtEditRequest()}>
