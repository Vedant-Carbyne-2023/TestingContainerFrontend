...
const PumpHouse = ({month, year}) => {
  // const [data, setData] = useState([]);

  const fetchData = async () => {
    try {
      const response = await api.post('/getPumpHouseDprAll');
      const responseData = response.data.data;
      console.log("API DATA",responseData);
  // setData(responseData)
  return responseData
      // Check if the response data has the expected structure
      // if (responseData && responseData.pumpHouseDpr && Array.isArray(responseData.pumpHouseDpr) && responseData.pumpHouseDpr.length > 0) {
      //   setData(responseData.pumpHouseDpr);
      // } else {
...
data.forEach((rowData, index) => {
  ...
  // Assuming there is a property named "pumpHouseDpr" in the API response containing daily data
  if (rowData.pumpHouseDpr) {
    rowData.pumpHouseDpr.forEach((dailyEntry, entryIndex) => {
      const date = moment(dailyEntry.dprDetails.pumpHouseDate).format('DD-MM-YYYY'); // Assuming there is a property named "date" in the API response
  ...
  return (
    <div>
      <button onClick={generateExcel} className='btn btn-primary'>Download Pump House Excel</button>
    </div>
  );
};
