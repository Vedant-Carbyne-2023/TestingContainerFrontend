...
const Oht = ({year, month}) => {
  const [data, setData] = useState([]);

  const fetchData = async () => {
    try {
      const response = await api.post('/getOhtDprAll');
      const responseData = response.data.data;
      console.log("API DATA",responseData);
  // setData(responseData)
  return responseData
      // Check if the response data has the expected structure
      // if (responseData && responseData.ohtDpr && Array.isArray(responseData.ohtDpr) && responseData.ohtDpr.length > 0) {
      //   setData(responseData.ohtDpr);
      // } else {
...
data.forEach((rowData, index) => {
  ...
  // Assuming there is a property named "ohtDpr" in the API response containing daily data
  if (rowData.ohtDpr) {
    rowData.ohtDpr.forEach((dailyEntry, entryIndex) => {
      const date = moment(dailyEntry.dprDetails.ohtDate).format('DD-MM-YYYY'); // Assuming there is a property named "date" in the API response
  ...
  return (
    <div>
      <button onClick={generateExcel} className='btn btn-primary'>Download Oht Excel</button>
    </div>
  );
};
