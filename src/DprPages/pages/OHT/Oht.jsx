...
export default function Oht({ projectName, gpName }) {

  const [vendorName, setVendorName] = useState('')
  const [oldOhtDprs, setOldOhtDprs] = useState([])
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      let updated = options.map((option) => ({
        ...option,
        disabled: !result.data.data || !result.data.data.ohtDpr || !result.data.data.ohtDpr.some(
      ...
      setOldOhtDprs(result.data.data && result.data.data.ohtDpr && result.data.data.ohtDpr)
      setLoading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    // console.log(total)
    console.log(projectName, gpName, formData.ohtDate, formData.labourCount, formData.reason, formData.remarks, formData.workDone, formData.cummulativeWork)

    // return

    if (!formData.labourCount) return alert('Labour Count Not Provided')
    formData.layout = formData.layout === "Yes" ? true : false;
    let result = api.post("/ohtDailyDpr",
      {
        projectName,
        gpName,
      ...
      <form onSubmit={handleSubmit}>
        <div className={styles.container}>
          <div className={styles.flexContainer}>
            ...
            <div className={styles.formGroup}>
              <label htmlFor="ohtDate" title="Select Date Of Dpr / Work Done"
                className={styles.formLabel}>Date Of Work</label>
              <input
                name="ohtDate"
                type="date"
                value={formData.ohtDate}
                onChange={(e) => handleChange(e, 'ohtDate')}
              />
            </div>
          </div>
          ...
          <div className={styles.flexContainer}>
            <div className={styles.formGroup}>
              ...
              title="Select Type Of Work Is Done"
                className={styles.formLabel}
              >Work Done</label>
              <select
                id="workDone"
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {oldOhtDprs && oldOhtDprs.length > 0 &&
                oldOhtDprs.map((dpr, index) => (
                  <tr key={index}>
                    <td>{moment(dpr.dprDetails.createdAt).format('DD/MM/YYYY')}</td>
                    <td>
                      {moment(
                        dpr.dprDetails.ohtDate
                          ? dpr.dprDetails.ohtDate
          ...
          <div className={styles.submitButton}>
            <button className={styles.btnPrimary} title="Click To Submit" type="submit">
              Submit
            </button>
          </div>
        </div>
